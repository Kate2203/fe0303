const i18n = {
    weeks: 'недель',
    days: 'дней',
    hours: 'часов',
    minutes: 'минут',
    seconds: 'секунд'
};

function makeCounterElement (counterProps) {
console.log(counterProps.data);
//console.log( Object.entries(counterProps.data));
const store={},
    title = createElement (
    'h2',
    {
    className: 'countdown__title'
    },
    counterProps.title
),
    counter= createElement(
        'div',
        {
            className: 'countdown__counter',
            children:  Object.entries(counterProps.data)
                .map(([name, value]) => {
                    
                    //console.log({name, value});
                    
                    store[name] = createElement (
                        'span',
                        {
                            className: 'countdown__value'
                        },
                        value
                    );

                    return createElement (
                        'div',
                        {
                            className: 'countdown item',
                            children: [
                                store[name],
                                createElement (
                                    'span',
                                    {
                                        className: 'countdown_label'
                                    },
                                    i18n[name] || name
                                )
                            ]
                        }
                    )   
                })
        }
    ),
    countdown = createElement (
    'div',
    {
        className: 'countdown',
        children: [
            title,
            counter
        ]
    }
);

store.title = title;
store.container = createElement (
    'div',
    {
        className: 'section',
        children: [
            countdown
        ]
    }
);
store.update = function(newProps) {
    if ('title' in newProps) {
        store.title.innerText = newProps.title;
    }

    if ('data' in newProps) {
        Object.entries(newProps.data)
            .forEach(([name, value]) => {
                if (store[name]) {
                    store[name].innerText = value;
                }
            });
    }
}
return store;
}

function getCountdownData ( destinationDate ) 
{
    let diff= destinationDate.getTime() - Date.now();
    //Date.now() - метод now объекта Date  возвращает текущее время в милисекундах
    //getTime() - возвращает тоже самое время в милисекундах

   return [
        ['weeks', getCountdownData.TIME_IN_WEEK],
        ['days', getCountdownData.TIME_IN_DAY],
        ['hours', getCountdownData.TIME_IN_HOUR],
        ['minutes', getCountdownData.TIME_IN_MIN],
        ['seconds', getCountdownData.TIME_IN_SEC]
    ].reduce(  // функция reduce (свертка) имеет 2 аргумента . 1-й - ф-я, к-рая будет выполняться для каждого элемента массива
        (result, [nameKey, timeInKey]) => { //2-й аргумент- начальное значение элемента, в который мы будем все сворачивать (в объект , в данном случае)
            result[nameKey] = Math.floor(diff / timeInKey);
            diff= diff % timeInKey;
            return result;
        }, 
        {}    
    );
}
// функция getCountdownData - тоже объект и у нее могут быть свойства. Называется статические свойства функции
getCountdownData.TIME_IN_SEC = 1000;
getCountdownData.TIME_IN_MIN = 60*getCountdownData.TIME_IN_SEC;
getCountdownData.TIME_IN_HOUR = 60*getCountdownData.TIME_IN_MIN;
getCountdownData.TIME_IN_DAY = 24*getCountdownData.TIME_IN_HOUR;
//getCountdownData.TIME_IN_WEEK = 7*24*60*60*1000;  7дней*24 часа в неделе* 60минут в часе*60 секунд в минуте*1000милисекунд в минуте 
getCountdownData.TIME_IN_WEEK = 7*getCountdownData.TIME_IN_DAY;

const rootEl = document.querySelector('#root')
const newYear = new Date(2021, 0 , 1)
const mayfirst = new Date (2020, 04, 01)
const karantin = new Date (2020, 03, 24) 
const counter1 = makeCounterElement (
    {
        title: 'До нового года осталось',
        data: getCountdownData (newYear) //сформировано время нового года
    });
const counter2 = makeCounterElement (
    {
        title: 'До 1 мая осталось',
        data: getCountdownData (mayfirst) 
    });
const counter3 = makeCounterElement (
        {
            title: 'До конца карантина осталось',
            data: getCountdownData (karantin) 
        });
    

console.log( counter1 );
render( [counter1.container, counter2.container, counter3.container], rootEl); // перерисовка счетчика , чтоб он обновлялся

setInterval(
    function () {
        counter1.update({
            data: getCountdownData (newYear)
        })
    },
    1000
)
setInterval(
    function () {
        counter2.update({
            data: getCountdownData (mayfirst)
        })
    },
    1000
)
setInterval(
    function () {
        counter3.update({
            data: getCountdownData (karantin)
        })
    },
    1000
)